...
def packet_generator():
    ...
    for n_packet in range(packet_want):
        ...
        print("Filter check:", firewall_filter(rule_2))
...
def writing_csv_plain():
    with open('packet_plain.csv', 'w', newline='') as myfile:
        wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
        for i in range(packet_want):
            wr.writerow(full_data[i])
            
def writing_csv_binary():
    convert_bin()
    with open('packet_binary.csv', 'w', newline='') as myfile:
        wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
        for i in range(packet_want):
            wr.writerow(full_data_bin[i])
