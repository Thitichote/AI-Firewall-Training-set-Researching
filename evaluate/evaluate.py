...
def evaluate_model():
    ...
    for count_test_set in range(1,4): # 1 - 3

        number_set = len(pd.read_csv('_csv/data_info.csv'))
        
        test_set_name = 'test_set_' + str(count_test_set)
        
        # put up test set path
        data = pd.read_csv('_csv/test_set/' + test_set_name +'.csv')
        
        test_x = data.iloc[:,1:data.shape[1]].values
        test_y = data.iloc[:,0].values
        
        # put all file are already generated
        for i in range(number_set):
            ...
            with open("_csv/result/evaluate_result.csv", 'a', newline='') as f:
                ...
                writer.writerow([model_name, evaluate_time, accuracy, true_positive, true_negative, false_positive, false_negative,test_set_name])
        
        print('evaluate', test_set_name + ' done..')